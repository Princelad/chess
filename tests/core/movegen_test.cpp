...
TEST(MoveGen, GameStateDrawPrecedenceOverCheckmate)
{
    // Checkmate position but halfmove clock at 100 — FIDE says draw rules take precedence.
    auto board = *Board::fromFen("4k3/8/8/8/8/8/r3q3/4K2R w - - 100 50");
...
TEST(MoveGen, InsufficientMaterialKnightVsKnight)
{
    // K+N vs K+N is NOT insufficient material
    auto board = *Board::fromFen("8/8/8/8/4n3/5N2/8/4K2k w - - 0 1");
    EXPECT_FALSE(insufficientMaterial(board));
}
...
TEST(MoveGen, IsLegalMovePromotion)
{
    auto board = *Board::fromFen("8/4P3/8/8/8/8/8/4K2k w - - 0 1");
    Square e7 = squareOf(File::E, Rank::R7);
    Square e8 = squareOf(File::E, Rank::R8);
    EXPECT_TRUE(isLegalMove(board, promotionMove(e7, e8, PieceType::Queen)));
    EXPECT_TRUE(isLegalMove(board, promotionMove(e7, e8, PieceType::Knight)));
    EXPECT_FALSE(isLegalMove(board, promotionMove(e7, e8, PieceType::King)));  // can't promote to king
}

TEST(MoveGen, EnPassantPinnedDiagonally)
{
    // White Ke5, white Pd5, black pc5 (EP target c6), black Ra5.
    // dxc6 EP would clear d5/c5, exposing Ke5 to Ra5 along rank 5 — should be illegal.
    auto board = *Board::fromFen("8/8/8/r1pPK3/8/8/8/8 w - c6 0 1");

    // EP is generated as pseudo-legal...
    auto pseudoLegal = generateMoves(board);
    bool hasEP = false;
    ...
    EXPECT_TRUE(hasEP);
    // ...but the legality filter correctly rejects it.
    EXPECT_FALSE(isLegalMove(board, enPassantMove(squareOf(File::D, Rank::R5), squareOf(File::C, Rank::R6))));
}
